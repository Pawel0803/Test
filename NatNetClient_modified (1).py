...
class NatNetClient:
    ...
    def __unpack_asset(self, data, major, minor, asset_num=0):
        ...
        def set_all_jsondata(self, jdata):
            self.alljsondata = jdata

        def set_pkt_time(self, t):
            self.pkt_time = t

        def write_to_file(self):
            import json
            with open(self.outputFile, 'w') as f:
                json.dump(self.alljsondata, f, indent=4)

        def processmessage(self, data):
            try:
                frame = self.unpack_frame_data(data)
                if frame is None:
                    return

                timestamp = getattr(self, 'pkt_time', int(time.time() * 1e9))

                json_frame = {
                    "timestamp_ns": timestamp,
                    "frame_number": getattr(frame, 'framenumber', -1),
                    "rigidBodies": [],
                    "markerSets": [],
                }

                for rb in getattr(frame, 'rigidBodies', []):
                    rigid_body = {
                        "id": rb.id,
                        "x": rb.x,
                        "y": rb.y,
                        ...
                        "qz": rb.qz,
                        "qw": rb.qw,
                        "mean_error": getattr(rb, 'mean_error', 0.0)
                    }
                    json_frame["rigidBodies"].append(rigid_body)

                for ms in getattr(frame, 'markerSets', []):
                    marker_set = {
                        "name": ms.name,
                        "markers": [{"x": m.x, "y": m.y, "z": m.z} for m in ms.markers]
                    }
                    json_frame["markerSets"].append(marker_set)

                if hasattr(self, 'alljsondata'):
                    self.alljsondata.append(json_frame)

            except Exception as e:
                print(f"[ERROR] Błąd w processmessage: {e}")
